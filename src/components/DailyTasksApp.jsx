...
const DailyTasksApp = ({ darkMode }) => {
  ...
  const getIncompleteTasksForDay = (day) => {
    return (
      scheduleData[day]?.filter(
        (task) => !isTaskCompleted(day, task.time, task.activity)
      ) || []
    );
  };
  ...
  const TaskListItem = ({ time, activity, completed, onTaskCompletion }) => (
    <div className="flex items-center justify-between px-4 py-2 text-sm sm:text-[16px] bg-white rounded-lg shadow-sm hover:bg-gray-50 transition-colors duration-200">
      <div className="flex flex-col items-start sm:flex-row sm:items-center sm:justify-center
  ...
  return (
    <div className={`max-w-4xl mx-auto p-6 bg-gray-100 rounded-lg shadow-md ${
      ...
      <div className="flex flex-col-reverse gap-4 sm:gap-0 sm:flex-row justify-start items-start sm:justify-between sm:items-center mb-6">
        ...
        <div className="flex items-center space-x-2 pl-1 text-gray-700 font-medium">
          <Calendar className="h-5 w-5" />
